...
config.unbind("d")
config.unbind("xo")
config.bind("m", 'hint links spawn umpv --ytdl-format="bestvideo[height<=720]+bestaudio" --script-opts=ytdl_hook-ytdl_path=yt-dlp "{hint-url}"')
...
## This setting supports URL patterns.
## [String]
c.content.headers.accept_language = "en-US,en,fr-FR,fr;q=0.5"
...
# STATUSBAR
#################################################################
## Widgets to display in the status bar
## [StringList]
c.statusbar.widgets = ["keypress", "search_match", "url", "scroll", "tabs", "progress", "clock:%H:%M:%S"]
